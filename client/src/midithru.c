...
#define AMPERO_NAME "Ampero Control MIDI 1"
#define DEFAULT_MIDI_INPUT_DEVICE 3
...
int main(int argc, char *argv[])
{
    if (argc >= 2) {
        ...
        if (argv[1][0] == '-' && argv[1][1] == 's') {
            for (int i = 0; i < 10; i++) {
                ...
                if (strcmp(info->name, AMPERO_NAME)!=0) continue;
                if (info->input == 0) continue;
                printf("id: %d\n", i);
                printf("name: %s\n", info->name);
    ...
    server_t server = tcp_connect("192.168.1.8", 8080);
    ...
    while (1) {
        ...
        while (Pm_Dequeue(in_queue, &buffer) == 1) {
            ...
            t = time(NULL);
            printf("[%s] %x\n", ctime(&t), buffer.message);
            tcp_send(server, buffer.message);
